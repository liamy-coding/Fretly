...
//
// 该缺陷在旧路径下被「首节点取 reps[0]」掩盖（代表点只影响候选排序，不进目标函数）；
// Round 2 引入「首节点取 positionCost 最小候选」后代表点进入 `cost`，缺陷被暴露：
// 三音和弦 [56,60,64]（G#3/C4/E4）作为小节首音被错选为 (1,0)(2,1)(4,6)（跨度 6），
// 而正确答案是 (1,0)(2,1)(3,1)（跨度 1）。
//
// ⚠️ 自 Round 5 起，和弦的绝对代价改用「该声部最高品位」而非代表点，代表点**不再进入
// 目标函数**（只用于相邻节点的转移代价）。因此下方 [56,60,64] 那条集成用例在
// 「bestDist 不刷新」的变异下**仍然通过**——它现在只是输出回归守卫，判别力已转移到
// 紧接着的「直接锁定语义」用例（后者直接断言 chordRepresentative 的 argmin 结果，
// 变异后必红；round 5 验收实测 1 failed / 23 passed）。
describe('把位代价 —— 和弦代表点 argmin 语义（bestDist 必须随 rep 刷新）', () => {
  const staff = (midi: number, tick = 0): StaffNote => ({ midi, tick, durationTick: 480 });

  it('判别性：[56,60,64] 作为小节首音 → 紧凑 voicing（跨度 ≤1 且最高 ≤1 品）', () => {
    // 历史行为（Round 2–4，和弦代价仍用代表点时且 bestDist 不刷新）：代表点落到
    // (4,6) → 首节点锚到该候选，输出 1:0 2:1 4:6（跨度 6）。
    // 当前（Round 5，改用声部最高品位基准）：输出 1:0 2:1 3:1。
    // 本用例守的是**输出**不退回宽松声部；bestDist 缺陷本身的判别由下一条 argmin
    // 用例承担（本用例在「bestDist 不刷新」变异下已不再变红）。
